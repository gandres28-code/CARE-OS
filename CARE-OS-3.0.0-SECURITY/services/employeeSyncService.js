...
const { codeLookup, protectCode, verifyCode } = require("./auth/credentialService");
...
async function upsertEmployee(employee) {
  ...
  const protectedCode = employee.code ? await protectCode(employee.code) : null;
  const sql = `
    ...
    VALUES ($1, $2, $3, NULL, $4, $5, $6, $7, $8, $9::jsonb, 'notion', NOW())
    ON CONFLICT (notion_id)
    DO UPDATE SET
      name = EXCLUDED.name,
      normalized_name = EXCLUDED.normalized_name,
      code = NULL,
      code_hash = COALESCE(EXCLUDED.code_hash, employees.code_hash),
      code_lookup = COALESCE(EXCLUDED.code_lookup, employees.code_lookup),
      role = EXCLUDED.role,
      ...
      source = 'notion',
      updated_at = NOW()
    RETURNING id, notion_id, name, role, hourly_rate, active,
      (code_hash IS NOT NULL AND code_hash <> '') AS has_code
  `;
  ...
  const result = await query(sql, [
    ...
    protectedCode?.codeHash || null,
    protectedCode?.codeLookup || null,
    employee.role,
    employee.hourlyRate,
    employee.active,
...
async function findEmployeeByCodePostgres(code) {
  ...
  const lookup = codeLookup(cleanCode);
  ...
  if (!employee || !(await verifyCode(cleanCode, employee.code_hash))) return null;
  delete employee.code_hash;
  return employee;
}
