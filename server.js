...
const { protectCode } = require("./services/auth/credentialService");
const {
  syncRoomsFromNotion,
  listRoomsPostgres,
  getRoomSyncStatus,
...
io.on("connection", (socket) => {
  ...
  const employeeRoom = `employee:${cleanEmployeeText(auth.employee_name || "")}`;
  if (auth.employee_name) socket.join(employeeRoom);
  if (normalizedRole.includes("cleaner")) socket.join("role:cleaner");
  if (normalizedRole.includes("inspector")) socket.join("role:inspector");
  if (normalizedRole.includes("runner")) socket.join("role:runner");
  if (/admin|manager|operations|dispatch|owner/.test(normalizedRole)) socket.join("role:operations");
  if (/admin|manager|payroll|owner/.test(normalizedRole)) socket.join("role:payroll");
});

function emitOperations(eventName, payload) {
  io.to("role:operations").emit(eventName, payload);
}

function emitPayroll(eventName, payload) {
  io.to("role:payroll").emit(eventName, payload);
}

function emitEmployee(eventName, employeeName, payload) {
  const room = `employee:${cleanEmployeeText(employeeName || "")}`;
  io.to(room).to("role:operations").emit(eventName, payload);
}

function emitServiceOrder(eventName, order) {
  const assigned = String(order?.assigned_to || order?.assignedTo || "").trim();
  const generic = !assigned || ["runner", "unassigned", "sin asignar"].includes(assigned.toLowerCase());
  let channel = io.to("role:operations");
  channel = generic ? channel.to("role:runner") : channel.to(`employee:${cleanEmployeeText(assigned)}`);
  channel.emit(eventName, order);
}
...
function addNotification(title, message) {
  ...
  emitOperations("system-notification", notification);
}
...
function broadcastAssignmentUpdate(reason = "notion-change", extra = {}) {
  ...
  io.to("role:operations").to("role:cleaner").to("role:inspector").emit("assignments-updated", payload);
  io.to("role:operations").to("role:cleaner").to("role:inspector").emit("rooms-updated", payload);
}
...
if (systemTimeline.length > 300) {
  ...
  emitOperations("ops-update", event);
  ...
  emitOperations("system-notification", systemNotifications[0]);
}
...
app.get("/healthz", (_req, res) => {
  res.json({ ok:true, service:"care-os", version:require("./package.json").version });
});

app.get("/readyz", (_req, res) => {
  const production = process.env.NODE_ENV === "production";
  const configuration = production ? validateProductionConfig(process.env) : {ok:true,errors:[]};
  const ready = configuration.ok && postgresStatus.connected;
  res.status(ready ? 200 : 503).json({
    ok:ready,
    postgres:{configured:postgresStatus.configured,connected:postgresStatus.connected},
    configuration:{ok:configuration.ok,errorCount:configuration.errors.length},
  });
});
...
app.post("/login-role", async (req, res) => {
  try {
    ...
    let user = await authenticateEmployeeCode(login);
    if (!user && String(process.env.ALLOW_LEGACY_NAME_LOGIN || "false").toLowerCase() === "true") {
      const employee = await findEmployeeByLogin(login);
      user = employee ? pageToUser(employee) : null;
    }
...
app.post("/refresh-employees", async (req, res) => {
  try {
    ...
    emitOperations("employees-updated", payload);
...
app.post("/api/auth/reauth", async (req, res) => {
  try {
    if (!req.auth || !req.sessionToken) return res.status(401).json({ ok:false, message:"Sesión requerida" });
    const attemptIdentity = req.auth.employee_name || req.auth.employee_id || "session";
    if (loginLocked(req, attemptIdentity)) return res.status(429).json({ok:false,message:"Demasiados intentos. Espera antes de volver a intentar."});
    const submitted = String(req.body?.code || "").trim();
    if (!submitted) return res.status(400).json({ ok:false, message:"Escribe tu código" });
    const verifiedUser = await authenticateEmployeeCode(submitted);
    const sameEmployee = verifiedUser && (
      (req.auth.employee_id && String(verifiedUser.id) === String(req.auth.employee_id)) ||
      cleanEmployeeText(verifiedUser.name) === cleanEmployeeText(req.auth.employee_name)
    );
    if (!sameEmployee) {
      recordLoginFailure(req, attemptIdentity);
      await SessionService.audit({ session:req.auth, action:"REAUTH_FAILED", targetType:"session", targetId:String(req.auth.id), request:{ ip:req.ip, requestId:req.requestId } });
      return res.status(401).json({ ok:false, message:"Código incorrecto" });
    }
    clearLoginFailures(req, attemptIdentity);
    const session = await SessionService.markReauthenticated(req.sessionToken);
    await SessionService.audit({ session, action:"REAUTH_SUCCESS", targetType:"session", targetId:String(session?.id || ""), request:{ ip:req.ip, requestId:req.requestId } });
    return res.json({ ok:true, verifiedAt:session?.reauthenticated_at || new Date().toISOString() });
  } catch (error) {
    console.error("REAUTH ERROR:", error.message);
    return res.status(500).json({ ok:false, message:"No se pudo confirmar la identidad" });
  }
});
app.get("/api/security/employee-code-status", async (_req, res) => {
  try {
    const result = await postgresQuery(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE code_hash IS NOT NULL AND code_hash <> '')::int AS protected, COUNT(*) FILTER (WHERE code IS NOT NULL AND code <> '')::int AS plaintext FROM employees`);
    res.json({ ok:true, ...result.rows[0] });
  } catch (error) {
    res.status(500).json({ ok:false, message:error.message });
  }
});
app.post("/api/security/migrate-employee-codes", async (req, res) => {
  try {
    let notionResult = null;
    if (NOTION_EMPLOYEES_DATABASE_ID) {
      notionResult = await syncEmployeesFromNotion({ notion, databaseId:NOTION_EMPLOYEES_DATABASE_ID });
    }
    const legacy = await postgresQuery(`SELECT id, code FROM employees WHERE code IS NOT NULL AND code <> ''`);
    let migratedLegacy = 0;
    for (const employee of legacy.rows) {
      const credential = await protectCode(employee.code);
      await postgresQuery(`UPDATE employees SET code=NULL, code_hash=$2, code_lookup=$3, updated_at=NOW() WHERE id=$1`, [employee.id, credential.codeHash, credential.codeLookup]);
      migratedLegacy += 1;
    }
    await SessionService.audit({session:req.auth,action:"EMPLOYEE_CODES_MIGRATED",targetType:"employees",details:{migratedLegacy,notionSaved:notionResult?.saved||0},request:{ip:req.ip,requestId:req.requestId}});
    res.json({ok:true,migratedLegacy,notion:notionResult});
  } catch (error) {
    console.error("EMPLOYEE CODE MIGRATION ERROR:", error.message);
    res.status(error.code==="23505"?409:500).json({ok:false,message:error.code==="23505"?"Hay códigos duplicados; corrígelos antes de migrar.":error.message});
  }
});
app.get("/app", (req, res) => {
...
async function runRoomSync(reason = "manual", date = todayISO()) {
  ...
  try {
    ...
    io.to("role:operations").to("role:cleaner").to("role:inspector").emit("rooms-postgres-synced", payload);
    io.to("role:operations").to("role:cleaner").to("role:inspector").emit("data-api-updated", {
      module: "rooms",
      date,
      reason,
...
async function runPayrollSync(reason = "manual", weekStart = "", weekEnd = "") {
  ...
  try {
    ...
    emitPayroll("payroll-postgres-synced", payload);
...
function postgresEmployeeToUser(employee) {
  const role = String(employee?.role || "");
  return {
    id: employee?.notion_id || employee?.id || "",
    postgresId: employee?.id || null,
    name: String(employee?.name || ""),
    role,
    active: employee?.active !== false,
    hotel: "Default Hotel",
    permissions: Array.isArray(employee?.permissions) && employee.permissions.length
...
async function authenticateEmployeeCode(code) {
  const cleanCode = String(code || "").trim();
  if (!cleanCode) return null;

  if (postgresStatus.connected) {
    const postgresEmployee = await findEmployeeByCodePostgres(cleanCode);
    if (postgresEmployee) return postgresEmployeeToUser(postgresEmployee);
  }

  if (!NOTION_EMPLOYEES_DATABASE_ID) return null;
  const notionEmployee = await findEmployeeByCode(cleanCode);
  if (!notionEmployee) return null;

  // Compatibilidad temporal: el primer acceso válido desde Notion se migra
  // inmediatamente a scrypt/HMAC y nunca conserva el código legible en PostgreSQL.
  if (postgresStatus.connected) {
    await upsertEmployee(getEmployeeFromNotionPage(notionEmployee));
  }
  return pageToUser(notionEmployee);
}
...
app.post("/mobile-code-login", async (req, res) => {
  try {
    ...
    const user = await authenticateEmployeeCode(code);
    ...
    if (!user.active) return res.status(403).json({ ok: false, message: "Empleado inactivo" });
    await completeSecureLogin(req, res, user);
...
for (const page of matches) {
  ...
  io.to("role:operations").to("role:inspector").to(`employee:${cleanEmployeeText(officialCleaner || employee)}`).emit("room-updated", roomUpdatePayload);
  ...
  if (adminAlert) {
    ...
    emitOperations("admin-operation-alert", adminPayload);
...
app.post("/submit-report", upload.array("photos", 20), async (req, res) => {
  try {
    ...
    const assignmentType = action.toUpperCase().includes("INSPECTION") ? "inspector" : "cleaner";
    const actor = await assertAssignedRoomAccess(req, unit, assignmentType);
    const employee = actor.name;
    ...
    const invalidFile = files.find((file) => !["image/jpeg", "image/png", "image/webp", "image/heic", "image/heif"].includes(String(file.mimetype || "").toLowerCase()));
    if (invalidFile) return res.status(415).json({ ok: false, success: false, message: "Formato de fotografía no permitido" });
    const uploadResults = [];
...
// =========================================================
const operationsRealtimeIo = { emit(eventName, payload) { emitOperations(eventName, payload); return true; } };
const IntelligenceEngine = createIntelligenceEngine({
  io: operationsRealtimeIo,
});
...
const secureEventIo = {
  emit(eventName, payload) {
    const employeeRoom = `employee:${cleanEmployeeText(payload?.employee || payload?.person || "")}`;
    if (["room-updated", "rooms-updated", "assignments-updated", "data-api-updated"].includes(eventName)) {
      io.to("role:operations").to("role:inspector").to(employeeRoom).emit(eventName, payload);
      return true;
    }
    if (["operations-log-created", "system-event-created", "system-notification"].includes(eventName)) {
      io.to("role:operations").to(employeeRoom).emit(eventName, payload);
      return true;
    }
    emitOperations(eventName, payload);
    return true;
  },
};
...
async function updateOperationalRoomPostgres({
  ...
  io.to("role:operations").to("role:inspector").to(`employee:${cleanEmployeeText(employee)}`).emit("room-updated", payload);
  io.to("role:operations").to("role:inspector").to(`employee:${cleanEmployeeText(employee)}`).emit("rooms-updated", payload);
  io.to("role:operations").to("role:inspector").to(`employee:${cleanEmployeeText(employee)}`).emit("assignments-updated", payload);
  io.to("role:operations").to("role:inspector").to(`employee:${cleanEmployeeText(employee)}`).emit("data-api-updated", payload);
...
async function insertOperationalEventPostgres({
  ...
  if (event) {
    emitEmployee("operations-log-created", event.employee, {
      id: event.id,
...
function authenticatedActor(req) {
  return {
    id: req.auth?.employee_id || "",
    name: String(req.auth?.employee_name || "").trim(),
    role: String(req.auth?.role || "").trim(),
    permissions: Array.isArray(req.auth?.permissions) ? req.auth.permissions : [],
  };
}

function actorHasRole(actor, allowed) {
  const role = cleanEmployeeText(actor?.role || "");
  return allowed.some((value) => role.includes(value));
}

async function assertAssignedRoomAccess(req, unit, assignmentType) {
  const actor = authenticatedActor(req);
  if (!actor.name) {
    const error = new Error("Sesión de empleado requerida");
    error.status = 401;
    throw error;
  }
  if (actorHasRole(actor, ["admin", "manager", "operations", "dispatch"])) return actor;

  const allowedRoles = assignmentType === "inspector"
    ? ["inspector"]
    : ["cleaner"];
  if (!actorHasRole(actor, allowedRoles)) {
    const error = new Error(`Tu rol no permite acciones de ${assignmentType === "inspector" ? "inspección" : "limpieza"}`);
    error.status = 403;
    throw error;
  }

  const result = await postgresQuery(
    `SELECT assigned_cleaner, assigned_cleaners, assigned_inspector, assigned_inspectors, stayover
     FROM rooms WHERE work_date=$1::date AND normalized_room=$2 LIMIT 1`,
    [todayISO(), normalizeRoom(unit)]
  );
  const room = result.rows[0];
  if (!room) {
    const error = new Error("La habitación no existe en las asignaciones de hoy");
    error.status = 404;
    throw error;
  }
  if (assignmentType === "cleaner" && room.stayover) {
    const error = new Error("Esta unidad está marcada como Stayover y no puede iniciarse");
    error.status = 409;
    throw error;
  }
  const values = assignmentType === "inspector"
    ? [room.assigned_inspector, ...(Array.isArray(room.assigned_inspectors) ? room.assigned_inspectors : [])]
    : [room.assigned_cleaner, ...(Array.isArray(room.assigned_cleaners) ? room.assigned_cleaners : [])];
  const names = values.flatMap((value) => String(value || "").split(",")).map(cleanEmployeeText).filter(Boolean);
  if (!names.includes(cleanEmployeeText(actor.name))) {
    const error = new Error("Esta habitación no está asignada a tu cuenta");
    error.status = 403;
    throw error;
  }
  return actor;
...
app.post("/action", async (req, res) => {
  try {
    const { action, unit, note, photoUrl, eventId, requestId } = req.body;
    const actor = await assertAssignedRoomAccess(req, unit, "cleaner");
    ...
    return res.status(status).json({ success: false, retryable:status === 503, message: status === 503 ? "Estamos sincronizando. Intenta nuevamente en unos segundos." : error.message });
  }
});
...
app.post("/api/intelligence/quality/refresh", async (req, res) => {
  try {
    ...
    io.to("role:operations").to("role:inspector").emit("quality:intelligence-refreshed", result);
    return res.json(result);
  } catch (error) {
...
app.post("/api/quality/reviews", async (req, res) => {
  try {
    const actor = await assertAssignedRoomAccess(req, req.body?.unit, "inspector");
    const { unit, cleaner, scores = {}, issues = [], notes = "", status = "in_progress" } = req.body || {};
    const inspector = actor.name;
    if (!unit || !inspector) {
    ...
    io.to("role:operations").to("role:inspector").emit("quality:updated", { unit, inspector, overallScore, status: finalStatus, updatedAt: new Date().toISOString() });

    QualityEngine.refreshRoomMetric({ date, normalizedRoom })
      .then(() => QualityEngine.refreshEmployeeMetrics(date))
      .then(() => io.to("role:operations").to("role:inspector").emit("quality:intelligence-refreshed", { date, unit, cleaner }))
...
app.post("/inspector-action", async (req, res) => {
  try {
    const { action, unit, note, photoUrl, eventId, requestId } = req.body;
    const actor = await assertAssignedRoomAccess(req, unit, "inspector");
    const name = actor.name;
...
app.post("/api/push/test", async (req, res) => {
  try {
    const actor = authenticatedActor(req);
    const elevated = actorHasRole(actor, ["admin", "manager", "operations", "dispatch"]);
    const employeeName = elevated && req.body.employeeName ? String(req.body.employeeName).trim() : actor.name;
...
function saveEmployeeNotification(employee, payload) {
  ...
  emitEmployee("employee-notification", employee, item);
  return item;
}
...
app.get("/api/employee-notifications", (req, res) => {
  const actor = authenticatedActor(req);
  const elevated = actorHasRole(actor, ["admin", "manager", "operations", "dispatch"]);
  const employee = elevated && req.query.employee ? String(req.query.employee).trim() : actor.name;
  const key = notificationEmployeeKey(employee);
...
app.post("/api/employee-notifications/read", (req, res) => {
  const actor = authenticatedActor(req);
  const elevated = actorHasRole(actor, ["admin", "manager", "operations", "dispatch"]);
  const employee = elevated && req.body.employee ? String(req.body.employee).trim() : actor.name;
...
app.post("/api/payroll/manual-entry", async (req, res) => {
  try {
    ...
    emitPayroll("payroll-record-updated", record);
...
app.post("/api/payroll/rates", async (req, res) => {
  try {
    ...
    emitPayroll("payroll-rates-updated", result);
    res.json({ ok: true, ...result });
  } catch (error) {
    res.status(400).json({ ok: false, message: error.message });
  }
...
app.post("/api/payroll/week/close", async (req, res) => {
  try {
    ...
    emitPayroll("payroll-week-closed", week);
...
app.post("/api/payroll/week/reopen", async (req, res) => {
  try {
    ...
    emitPayroll("payroll-week-reopened", week);
    res.json({ ok: true, week });
  } catch (error) {
    res.status(400).json({ ok: false, message: error.message });
  }
...
app.get("/inspector-assignments", async (req, res) => {
  try {
    const actor = authenticatedActor(req);
    if (!actor.name) return res.status(401).json({ ok: false, message: "Sesión requerida" });
    if (!actorHasRole(actor, ["inspector", "admin", "manager", "operations", "dispatch"])) {
      return res.status(403).json({ ok: false, message: "Tu rol no tiene acceso a inspecciones" });
    }

    const date = todayISO();
    const ttlMs = Number(process.env.ASSIGNMENTS_CACHE_MS || 15000);
    const coreCacheKey = `core:inspector-assignments:${date}:${cleanEmployeeText(actor.name)}`;

    const payload = await OSCore.cache.remember(
      coreCacheKey,
      ttlMs,
      async () => {
        const inspectorName = actor.name;
        const role = actor.role;
        const canSeeAll = actorHasRole(actor, ["admin", "manager", "operations", "dispatch"]);
      ...
      [`assignments:${date}`, "assignments", `inspector:${cleanEmployeeText(actor.name)}`]
    );
...
app.get("/cleaner-assignments", async (req, res) => {
  try {
    ...
    if (!actor.name) return res.status(401).json({ ok: false, message: "Sesión requerida" });
    const elevated = actorHasRole(actor, ["admin", "manager", "operations", "dispatch"]);
    const name = elevated && req.query.name ? String(req.query.name).trim() : actor.name;
...
async function employeeForAuthenticatedSession(req) {
  const actor = authenticatedActor(req);
  if (!actor.name) return null;
  const employees = await getEmployeesCached();
  return employees.find((page) =>
    (actor.id && String(page.id) === String(actor.id)) ||
    cleanEmployeeText(getEmployeeNameFromPage(page)) === cleanEmployeeText(actor.name)
  ) || null;
}
...
app.post("/clock-in", async (req, res) => {
  try {
    ...
    const employee = await employeeForAuthenticatedSession(req);
    ...
    const code = getEmployeeCodeFromPage(employee);
    if (!code) return res.status(409).json({ ok: false, error: "Tu cuenta no tiene un código de Time Clock configurado." });
...
function employeeAdminPayload(body = {}) {
  const name = String(body.name || "").trim().slice(0, 160);
  const code = String(body.code || "").trim().slice(0, 80);
  const role = String(body.role || "Cleaner").trim().slice(0, 120);
  const payType = body.payType === "unit" ? "unit" : "hourly";
  const workLocations = Array.isArray(body.workLocations) ? body.workLocations.map(v => String(v || "").trim().slice(0,120)).filter(Boolean) : [];
  return { name, code, role, payType, hourlyRate:Math.max(0,Number(body.hourlyRate)||0), unitRate:Math.max(0,Number(body.unitRate)||0), phone:String(body.phone||"").trim().slice(0,80), email:String(body.email||"").trim().slice(0,180), workLocations, notes:String(body.notes||"").trim().slice(0,2000), active:body.active!==false };
}

app.post("/api/v2/employees", async (req, res) => {
  try {
    const item = employeeAdminPayload(req.body);
    if (!item.name || !item.code) return res.status(400).json({ok:false,message:"Nombre y código son obligatorios"});
    const credential = await protectCode(item.code);
    const result = await postgresQuery(`INSERT INTO employees(name,normalized_name,code,code_hash,code_lookup,role,hourly_rate,active,permissions,source,pay_type,unit_rate,phone,email,work_locations,notes) VALUES($1,$2,NULL,$3,$4,$5,$6,$7,'[]'::jsonb,'care-admin',$8,$9,$10,$11,$12::jsonb,$13) RETURNING id,name,role,active,hourly_rate AS "hourlyRate",pay_type AS "payType",unit_rate AS "unitRate",phone,email,work_locations AS "workLocations",notes,(code_hash IS NOT NULL AND code_hash<>'') AS "hasCode"`,[item.name,cleanEmployeeText(item.name),credential.codeHash,credential.codeLookup,item.role,item.hourlyRate,item.active,item.payType,item.unitRate,item.phone,item.email,JSON.stringify(item.workLocations),item.notes]);
    await SessionService.audit({session:req.auth,action:"EMPLOYEE_CREATED",targetType:"employee",targetId:String(result.rows[0].id),details:{name:item.name,role:item.role},request:{ip:req.ip,requestId:req.requestId}});
    emitOperations("employees-updated",{reason:"created",id:result.rows[0].id});
    res.status(201).json({ok:true,employee:result.rows[0]});
  } catch(error) { res.status(error.code==="23505"?409:500).json({ok:false,message:error.code==="23505"?"Ese código ya está en uso":error.message}); }
});

app.put("/api/v2/employees/:id", async (req, res) => {
  try {
    const id=Number(req.params.id),item=employeeAdminPayload(req.body);
    if(!Number.isInteger(id)||!item.name)return res.status(400).json({ok:false,message:"Empleado o nombre inválido"});
    const credential=item.code?await protectCode(item.code):null;
    const result=await postgresQuery(`UPDATE employees SET name=$2,normalized_name=$3,code=NULL,code_hash=CASE WHEN $4::text IS NULL THEN code_hash ELSE $4 END,code_lookup=CASE WHEN $5::text IS NULL THEN code_lookup ELSE $5 END,role=$6,hourly_rate=$7,active=$8,pay_type=$9,unit_rate=$10,phone=$11,email=$12,work_locations=$13::jsonb,notes=$14,updated_at=NOW() WHERE id=$1 RETURNING id,name,role,active,hourly_rate AS "hourlyRate",pay_type AS "payType",unit_rate AS "unitRate",phone,email,work_locations AS "workLocations",notes,(code_hash IS NOT NULL AND code_hash<>'') AS "hasCode"`,[id,item.name,cleanEmployeeText(item.name),credential?.codeHash||null,credential?.codeLookup||null,item.role,item.hourlyRate,item.active,item.payType,item.unitRate,item.phone,item.email,JSON.stringify(item.workLocations),item.notes]);
    if(!result.rows[0])return res.status(404).json({ok:false,message:"Empleado no encontrado"});
    await SessionService.audit({session:req.auth,action:"EMPLOYEE_UPDATED",targetType:"employee",targetId:String(id),details:{name:item.name,role:item.role,codeChanged:Boolean(item.code)},request:{ip:req.ip,requestId:req.requestId}});
    emitOperations("employees-updated",{reason:"updated",id});res.json({ok:true,employee:result.rows[0]});
  } catch(error){res.status(error.code==="23505"?409:500).json({ok:false,message:error.code==="23505"?"Ese código ya está en uso":error.message});}
});

app.patch("/api/v2/employees/:id/status", async (req,res)=>{
  try{const id=Number(req.params.id);const result=await postgresQuery(`UPDATE employees SET active=$2,updated_at=NOW() WHERE id=$1 RETURNING id,name,role,active`,[id,req.body.active===true]);if(!result.rows[0])return res.status(404).json({ok:false,message:"Empleado no encontrado"});await SessionService.audit({session:req.auth,action:req.body.active===true?"EMPLOYEE_ACTIVATED":"EMPLOYEE_DEACTIVATED",targetType:"employee",targetId:String(id),details:{name:result.rows[0].name},request:{ip:req.ip,requestId:req.requestId}});emitOperations("employees-updated",{reason:"status",id});res.json({ok:true,employee:result.rows[0]});}catch(error){res.status(500).json({ok:false,message:error.message});}
});
    ...
    const result = await processNextSyncJob({
      ...
      processors: {
        "notion-room-action": async (payload) => {
          ...
          emitEmployee("notion-action-synced", payload.name, {
            ok: true,
            action: payload.action,
            unit: payload.unit,
...
app.post("/api/communications/announcements", async (req, res) => {
  try {
    ...
    emitOperations("communications:announcement",result.rows[0]);
...
app.patch("/api/communications/announcements/:id", async (req,res)=>{
  try{
    ...
    emitOperations("communications:updated",{type:"announcement",id:Number(req.params.id)});
...
app.post("/api/rooms/:roomNumber/catalog/photos", upload.single("photo"), async (req,res)=>{
  try{
    ...
    emitOperations("room-catalog:updated",{roomNumber,type:"photo"});
...
app.post("/api/rooms/:roomNumber/catalog/inventory", async (req,res)=>{
  try{
    ...
    emitOperations("room-catalog:updated",{roomNumber,type:"inventory"});
...
app.get("/api/service-orders", async (req, res) => {
  try {
    ...
    const where = [];
    const actor = authenticatedActor(req);
    const runnerOnly = actorHasRole(actor, ["runner"]) && !actorHasRole(actor, ["admin", "manager", "operations", "dispatch"]);
    ...
    if (runnerOnly) {
      ...
      where.push(`(LOWER(COALESCE(assigned_to,'')) IN ('','runner','unassigned','sin asignar') OR LOWER(assigned_to)=LOWER($${values.length}))`);
    }
    const result = await postgresQuery(`
...
app.get("/api/service-orders/:id", async (req, res) => {
  try {
    ...
    const actor = authenticatedActor(req);
    const runnerOnly = actorHasRole(actor, ["runner"]) && !actorHasRole(actor, ["admin", "manager", "operations", "dispatch"]);
    const assigned = String(orderResult.rows[0].assigned_to || "");
    if (runnerOnly && assigned && !["runner","unassigned","sin asignar"].includes(assigned.toLowerCase()) && cleanEmployeeText(assigned) !== cleanEmployeeText(actor.name)) {
      return res.status(403).json({ ok:false, error:"Esta orden está asignada a otro runner." });
    }
...
app.post("/api/service-orders", async (req, res) => {
  ...
  try {
    ...
    emitServiceOrder("service-order:created", order);
    ...
    const adminOrderAlert = {
      ...
      emitOperations("admin-operation-alert", adminOrderAlert);
    Promise.resolve(sendPushToRole(postgresQuery, "admin", {
...
app.patch("/api/service-orders/:id/status", async (req, res) => {
  ...
  try {
    ...
    const sessionActor = authenticatedActor(req);
    const actor = sessionActor.name;
    ...
    await client.query("BEGIN");
    const existing = await client.query("SELECT * FROM service_orders WHERE id=$1 FOR UPDATE", [id]);
    if (!existing.rows[0]) { await client.query("ROLLBACK"); return res.status(404).json({ ok:false, error:"Orden no encontrada." }); }
    const runnerOnly = actorHasRole(sessionActor, ["runner"]) && !actorHasRole(sessionActor, ["admin", "manager", "operations", "dispatch"]);
    const assigned = String(existing.rows[0].assigned_to || "").trim();
    const generic = !assigned || ["runner","unassigned","sin asignar"].includes(assigned.toLowerCase());
    if (runnerOnly && !generic && cleanEmployeeText(assigned) !== cleanEmployeeText(actor)) {
      await client.query("ROLLBACK");
      return res.status(403).json({ ok:false, error:"Esta orden está asignada a otro runner." });
    }
    if (runnerOnly && !["accepted","in_progress","delivered","completed"].includes(status)) {
      await client.query("ROLLBACK");
      return res.status(403).json({ ok:false, error:"Tu rol no puede usar ese estado." });
    }
    if (runnerOnly) {
      const nextByStatus = { new:"accepted", accepted:"in_progress", in_progress:"delivered", delivered:"completed" };
      if (nextByStatus[String(existing.rows[0].status || "new")] !== status) {
        await client.query("ROLLBACK");
        return res.status(409).json({ ok:false, error:"La orden debe avanzar en el orden establecido." });
      }
    }
    const sql = `UPDATE service_orders SET status=$1, updated_at=NOW()${timestampColumn ? `, ${timestampColumn}=COALESCE(${timestampColumn},NOW())` : ""}${actor && status==='accepted' ? ", assigned_to=CASE WHEN LOWER(COALESCE(assigned_to,'')) IN ('','runner','unassigned','sin asignar') THEN $3 ELSE assigned_to END" : ''} WHERE id=$2 RETURNING *`;
    ...
    await client.query("COMMIT");
    const order = normalizeServiceOrder(result.rows[0]);
    await SessionService.audit({session:req.auth,action:"SERVICE_ORDER_STATUS_CHANGED",targetType:"service_order",targetId:String(id),details:{from:existing.rows[0].status,to:status},request:{ip:req.ip,requestId:req.requestId}});
    emitServiceOrder("service-order:updated", order);
...
app.patch("/api/admin/rooms/:id", async (req, res) => {
  ...
  try {
    ...
    const roomChannels = io.to("role:operations").to("role:inspector").to(`employee:${cleanEmployeeText(room.assignedCleaner || "")}`);
    roomChannels.emit("room-updated", payload);
    roomChannels.emit("rooms-updated", payload);
    roomChannels.emit("assignments-updated", payload);
...
startServer().catch((error) => {
  ...
  // En desarrollo se conserva un modo de respaldo para facilitar diagnóstico local.
  server.listen(PORT, () => {
